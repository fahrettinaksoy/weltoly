...
const regexXmlParse: XmlParse = (text): XmlDoc => {
  ...
  const currencies = [
    ...text.matchAll(/<Currency[^>]*Kod="([^"]+)"[^>]*>([\s\S]*?)<\/Currency>/g)
  ].map((m) => ({
    code: m[1] ?? null,
...
describe('parseFrankfurter', () => {
  ...
  it('base USD değilse reddeder — matematik tutmaz', () => {
    const r = parseFrankfurter(
      JSON.stringify({ base: 'EUR', date: '2026-07-16', rates: { USD: 1.15 } })
    )
...
describe('parseTcmb — TRY tabanından USD tabanına', () => {
  ...
  it('uSD yoksa reddeder — tabana çevrilemez', () => {
    const noUsd =
      '<Tarih_Date Tarih="16.07.2026"><Currency Kod="EUR"><Unit>1</Unit><ForexSelling>53.95</ForexSelling></Currency></Tarih_Date>'
