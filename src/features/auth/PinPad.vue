...
const props = withDefaults(
  defineProps<{ title?: string, subtitle?: string, error?: boolean, length?: number, disabled?: boolean }>(),
  { length: 6 },
)
...
function push(d: string) {
  if (props.disabled || buffer.value.length >= props.length)
    return
  buffer.value += d
  if (buffer.value.length === props.length)
...
<template>
  <div class="pinpad text-center">
    ...
    <div class="keys">
      <v-btn v-for="k in keys" :key="k" variant="tonal" size="large" class="key" :disabled="disabled" @click="push(k)">
      ...
      <v-btn variant="text" size="large" class="key" icon="mdi-backspace-outline" :disabled="disabled" @click="back" />
    </div>
  </div>
</template>
