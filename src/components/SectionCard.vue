  ...
  <v-card color="surface-light" variant="flat" :elevation="0">
    <div class="section-header d-flex align-center ga-3 px-4 py-3">
      ...
      <div class="overflow-hidden">
        <div class="text-title-small font-weight-bold text-truncate">{{ title }}</div>
        <div v-if="subtitle" class="text-body-small text-medium-emphasis text-truncate">{{ subtitle }}</div>
      </div>
      <v-spacer />
      <slot name="actions" />
    </div>
